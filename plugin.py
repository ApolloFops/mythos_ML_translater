...
journal.log(f"Using device: {device}", 6, component=LOG_COMPONENT)

class MythosMLTranslater(commands.Cog):
	model = None
	tokenizer = None

	command_group = discord.SlashCommandGroup("mythosml")
	...
	@command_group.command(name="train", description="Trains the MythosML machine learning model.", contexts=CONTEXTS, integration_types=INTEGRATION_TYPES)
	@isDeveloper()
	async def train_command(self, ctx: discord.ApplicationContext):
		await ctx.defer()

		self.train_model()

		await ctx.respond("Finished training!")
